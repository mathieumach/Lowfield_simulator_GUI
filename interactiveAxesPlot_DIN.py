...
class interactivePlot(object):   
    def __init__(self,fig, ax, X, TR, TE, TI, TI2, T1, T2, M0, B1, noise, data_mat, plotAxis = 2, axisLabels = None, fov = None, cmap = 'gray'):
...
#        set various event handlers
        ...
        def planefunc(label):
            ...
            if self.plane == 'cross line':
                ...
                imageDataX = np.rot90(self.X[self.x_ind,:,:])
                s = np.rot90(self.X[self.x_ind,:,:]).shape
                arr1 = np.ones(s[0])
                arr2 = np.ones(s[1])
