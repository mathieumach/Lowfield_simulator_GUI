...
#        set various event handlers
        ...
        if self.plotAxis == 0:
            ...
            imageDatay = self.filterimages[self.ind,:,:]
        elif self.plotAxis == 1:
            imageDatax = self.X[:,self.ind,:]
            imageDatay = self.filterimages[:,self.ind,:]
        elif self.plotAxis == 2:
            imageDatax = self.X[:,:,self.ind]
            imageDatay = denoise_nl_means(self.filterimages[:,:,self.ind], h=h * sigma_estim, sigma=sigma_estim, fast_mode=False, patch_size=s, patch_distance=d)
        else:
            print("invalid axis")
            return -1
        ...
        # Global title
        self.fig.suptitle("Axial axe", fontsize=14)
        ...
        def update(val):
            ...
            if self.plotAxis == 0:
                self.filterimages[self.ind,:,:] = denoise_nl_means(self.original[self.ind,:,:], h=hh * sigma_estim, sigma=sigma_estim, fast_mode=False, patch_size=ss, patch_distance=dd)
            if self.plotAxis == 1:
                self.filterimages[:,self.ind,:] = denoise_nl_means(self.original[:,self.ind,:], h=hh * sigma_estim, sigma=sigma_estim, fast_mode=False, patch_size=ss, patch_distance=dd)
            elif self.plotAxis == 2:
                self.filterimages[:,:,self.ind] = denoise_nl_means(self.original[:,:,self.ind], h=hh * sigma_estim, sigma=sigma_estim, fast_mode=False, patch_size=ss, patch_distance=dd)
    ...
    def keyPress(self, event):
        ...
        if event.key == " ": #change orientation on space bar press
            self.plotAxis += 1
            if self.plotAxis == 1:
                self.fig.suptitle("Coronal axe", fontsize=14)
                self.filterimages[:,self.ind,:] = denoise_nl_means(self.original[:,self.ind,:], h=hh * self.sigma_estim, sigma=self.sigma_estim, fast_mode=False, patch_size=ss, patch_distance=dd)
            elif self.plotAxis == 2:
                self.fig.suptitle("Axial axe", fontsize=14)
                self.filterimages[:,:,self.ind] = denoise_nl_means(self.original[:,:,self.ind], h=hh * self.sigma_estim, sigma=self.sigma_estim, fast_mode=False, patch_size=ss, patch_distance=dd)
            if self.plotAxis > 2:
                self.plotAxis = 0
                self.fig.suptitle("Sagittal axe", fontsize=14)
                self.filterimages[self.ind,:,:] = denoise_nl_means(self.original[self.ind,:,:], h=hh * self.sigma_estim, sigma=self.sigma_estim, fast_mode=False, patch_size=ss, patch_distance=dd)
            self.updateSlice()
    ...
    def updateSlice(self):
        if self.plotAxis == 0:
            ...
            imageDataY = np.rot90(self.filterimages[self.ind,:,:])
            if self.axisLabels != None:
                self.ax.set_xlabel(self.axisLabels[2])
                self.ax.set_ylabel(self.axisLabels[1])
